...
apt-get install -yqq software-properties-common curl gpgv2 build-essential python3 make perl gcc net-tools bsdmainutils util-linux
...
sudo -u "${user}" /usr/bin/composer install --no-scripts --no-progress --optimize-autoloader
